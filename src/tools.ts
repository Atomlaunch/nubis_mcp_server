...
export type ToolExecutor = (request: {
  endpoint: string;
  schema: any;
}) => Promise<{ data: any; api_usage?: unknown }>;

/** A fresh server per transport/user. Credentials belong to the injected executor. */
export function createNubisMcpServer(
  getResultsFromMiddleware: ToolExecutor,
  allowTool: (name: string) => boolean = () => true,
): McpServer {
  ...
  const mcpBoardEnum = z
    ...
    .describe(
      "Kanban board. Live keys: inbox, priority, bugs, in-progress, reviewing, done, closed. Aliases: backlog→inbox, completed→done.",
    );

  for (const tool of ["get_projects", "get_task_boards"] as const) {
    if (allowTool(tool))
      server.tool(
        tool,
        tool === "get_projects"
          ? "List projects in this workspace. Use their IDs with task tools."
          : "List project workflow boards and ordered columns. Use column UUIDs for exact task placement; behavior identifies completion.",
    ...
    server.tool(
      ...
      "List tasks from the workspace kanban board. Returns task details including title, description, board status, GitHub file references, blockers, and images. Use board filter to see tasks by status, bolt_id to filter by project area. Start here to find tasks to work on.",
      {
        ...
        board_column_id: z
          ...
          .optional()
          .describe(
            "Exact workflow column UUID; takes precedence over legacy board status",
          ),
        limit: z
          .number()
          .optional()
      ...
      async ({
        ...
        try {
          ...
          type Task = {
            ...
            readonly board_column_id?: string;
    ...
    server.tool(
      ...
      "Save implementation notes, code context, or developer notes to a task. Use this to document decisions, add code snippets, or save progress notes that will help future work on this task. Context is appended (not replaced).",
      {
        ...
        context: z
          .string()
          .describe(
            "Text content to save - can include code snippets, notes, decisions, or any relevant information",
          ),
      },
  ...
  if (allowTool("work_on_task")) {
    server.tool(
      ...
      async ({ taskID }) => {
        ...
        if (
          Array.isArray(taskData.data.pm_task_blockers) &&
  ...
  if (allowTool("move_task")) {
    server.tool(
      "move_task",
      "Move a task to a different kanban board column. Live keys: inbox, priority, bugs, in-progress, reviewing, done, closed. Aliases: backlog→inbox, completed→done.",
      {
        project_id: z
          .string()
          .uuid()
    ...
    server.tool(
      ...
      "Create a new task or subtask in the workspace. Tasks are created in inbox by default. Link to GitHub files/directories to associate code with tasks. Use parent_task_id to create subtasks under a parent task.",
      {
        ...
        description: z
          ...
          .optional()
          .describe(
            "Detailed description, acceptance criteria, or implementation notes",
          ),
        board: mcpBoardEnum
          .optional()
          .describe(
            "Initial board placement (default: inbox; backlog maps to inbox)",
          ),
        parent_task_id: z
          .string()
          .optional()
        ...
        github_repo_name: z
          ...
          .optional()
          .describe(
            "Repository in format 'owner/repo', e.g., 'Atomlaunch/nubis'",
          ),
        bolt_id: z
          .string()
          .optional()
  ...
  if (allowTool("add_blocker")) {
    server.tool(
      "add_blocker",
      "Mark a task as blocked by another task. The blocked task cannot be worked on until the blocker is resolved (moved to done).",
      {
  ...
  function jsonToolContent(json: { data: unknown; api_usage?: unknown }) {
    return {
      content: [
        {
          type: "text" as const,
  ...
  if (allowTool("list_agent_memberships")) {
    server.tool(
      ...
      "List workspaces this agent principal is a member of (own pm_members rows for auth.uid()).",
      {},
      async () => {
        const json = await getResultsFromMiddleware({
  ...
  if (allowTool("mint_agent")) {
    server.tool(
      "mint_agent",
      "Mint a new agent principal in this workspace via Edge invite-agent. Agent admin only; new agents are members (not admin/owner). Human owner/admin mint and rotate/revoke in Settings. Returns the plaintext key once — do not log it. 501 until invite-agent is wired.",
      {
        display_name: z
          .string()
          .optional()
