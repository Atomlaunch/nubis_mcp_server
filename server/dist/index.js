...
async function checkUserApiKey(apiKey, workspaceId) {
    if (!workspaceId)
    ...
    const { data, error } = await supabase
        ...
        .single();
    if (error || !data)
        throw new Error("Invalid or unauthorized apiKey");
    const { user_id } = data;
...
app.post("/get_tasks", async (req, res) => {
    const { workspaceId, apiKey, schema } = req.body;
...
app.post("/get_task", async (req, res) => {
    const { workspaceId, apiKey, taskID } = req.body;
 ...
 */
app.post("/get_task_images", async (req, res) => {
    const { workspaceId, apiKey, taskID } = req.body;
    console.log({ workspaceId, apiKey, taskID });
    const checkUserApiKeyResult = await checkUserApiKey(apiKey, workspaceId);
    if (!checkUserApiKeyResult) {
 ...
 * Return work on task
 */
app.post("/work_on_task", async (req, res) => {
    const { workspaceId, apiKey, taskID } = req.body;
    console.log({ workspaceId, apiKey, taskID });
    const checkUserApiKeyResult = await checkUserApiKey(apiKey, workspaceId);
    if (!checkUserApiKeyResult) {
 ...
 */
app.post("/explain_setup", async (req, res) => {
    const { workspaceId, apiKey, taskID } = req.body;
    console.log({ workspaceId, apiKey, taskID });
    const checkUserApiKeyResult = await checkUserApiKey(apiKey, workspaceId);
    if (!checkUserApiKeyResult) {
...
app.post("/move_task", async (req, res) => {
    const { workspaceId, apiKey, taskID, board } = req.body;
